// The csapp.c and csapp.h is provided by csapp course group.
//
//  main.c
//  proxy_project
//
...
void func3(pool *p, int i)
{
    if(IsinSet(p->server_fd[i],&p->ready_read_set)){
		...
		int n;
		while((n = rio_readlineb(&p->serverrio[i], buf,MAXLINE)) > 0)
		{
		...
		switch (n) {
        	...
        	case 0:
            	break;
        	default:
                printf("********READ ENDSERVER STATE LEAK:The clientfd = %d,the endserver = %d \n ",cfd,sfd);
            	break;
		}
		Terminate_work(p, i);
    }
